...
os.environ["CUDA_VISIBLE_DEVICES"] = ",".join(
    str(gpu) for gpu in vars(config)["gpu_idx"])
...
class Trainer():
    ...
    def __init__(self, dataloader, hierarchical_transformer, config, i):
        ...
        self.word_pos_encoder = PositionEncoder.PositionEncoder(
            config, self.config.max_length)
        self.time_delay_encoder = PositionEncoder.PositionEncoder(
            config, self.config.size)

        # <----------- Check for GPU setting ----------->
        if self.config.gpu:

            self.hierarchical_transformer = DataParallelModel(
                hierarchical_transformer.cuda())
        ...
        self.adam_optimizer = optim.Adam(self.hierarchical_transformer.parameters(), np.power(
            self.config.d_model, - 0.5), betas=(self.config.beta_1, self.config.beta_2))
    ...
    def test_performance(self, type_):
        ...
        with torch.no_grad():
            ...
            for X, y, word_pos, time_delay, structure, attention_mask_word, attention_mask_post in self.dataloader.get_data(type_):
                ...
                predicted_y = self.hierarchical_transformer(
                    X, word_pos, time_delay, structure, attention_mask_word=attention_mask_word, attention_mask_post=attention_mask_post)
    ...
    def train(self):
        ...
        name = "{}_split_{}_{}".format(dataset, self.iter, datetime.now().strftime(
            '%Y-%m-%d-%H:%M:%S'))  # Date & Time for logging purposes
        path = os.path.join(self.config.log_folder, self.config.dataset_name,
                            name + "_" + self.config.experiment_name)
        ...
        for epoch in tqdm(range(self.config.num_epoch)):
            ...
            for X, y, word_pos, time_delay, structure, attention_mask_word, attention_mask_post in self.dataloader.get_data("train"):
                ...
                # <------- Settings ------------->
                # Set the model to be on train mode (So that the dropout applies)
                self.hierarchical_transformer.train()
            ...
            with torch.no_grad():
                ...
                acc_train, pre_train, recall_train, f_score_train, counter_true_train, counter_pred_train = cal_scores(
                    pred_train, true_train, type_="all")
                acc_test_1, pre_test_1, recall_test_1, f_score_test_1, counter_true_test_1, counter_pred_test_1 = cal_scores(
                    pred_test_1, true_test_1, type_="all")
                acc_test_2, pre_test_2, recall_test_2, f_score_test_2, counter_true_test_2, counter_pred_test_2 = cal_scores(
                    pred_test_2, true_test_2, type_="all")
                acc_test, pre_test, recall_test, f_score_test, counter_true_test, counter_pred_test = cal_scores(
                    pred_test, true_test, type_="all")

                # <-------- Getting performance for individual claseses  -------->
                acc_test_1_class_0, pre_test_1_class_0, recall_test_1_class_0, f_score_test_1_class_0, counter_true_test_1_class_0, counter_pred_test_1_class_0 = cal_scores(
                    pred_test_1, true_test_1, type_=0)
                acc_test_1_class_1, pre_test_1_class_1, recall_test_1_class_1, f_score_test_1_class_1, counter_true_test_1_class_1, counter_pred_test_1_class_1 = cal_scores(
                    pred_test_1, true_test_1, type_=1)

                acc_test_2_class_0, pre_test_2_class_0, recall_test_2_class_0, f_score_test_2_class_0, counter_true_test_2_class_0, counter_pred_test_2_class_0 = cal_scores(
                    pred_test_2, true_test_2, type_=0)
                acc_test_2_class_1, pre_test_2_class_1, recall_test_2_class_1, f_score_test_2_class_1, counter_true_test_2_class_1, counter_pred_test_2_class_1 = cal_scores(
                    pred_test_2, true_test_2, type_=1)

                acc_test_class_0, pre_test_class_0, recall_test_class_0, f_score_test_class_0, counter_true_test_class_0, counter_pred_test_class_0 = cal_scores(
                    pred_test, true_test, type_=0)
                acc_test_class_1, pre_test_class_1, recall_test_class_1, f_score_test_class_1, counter_true_test_class_1, counter_pred_test_class_1 = cal_scores(
                    pred_test, true_test, type_=1)
                ...
                if f_score_test_1 >= best_f_score_test_1:
                    ...
                    print(
                        "CURRENT BEST (F-SCORE) FOUND AT EPOCH : {} (EVALUATED WITH {})".format(epoch + 1, "TEST_1"))
                    ...
                    if f_score_test_1 == best_f_score_test_1:
                        ...
                        if f_score_test_2 >= best_f_score_test_1_for_2:
                            ...
                            log_best_model_info(
                                path, "epoch : " + str(epoch + 1), best_record_f_score_test_1, "f_score", "test_1")
                            save_best_model(path, self.hierarchical_transformer, self.word_encoder,
                                            self.word_pos_encoder, self.time_delay_encoder, self.optimizer, "f_score", "test_1")
                ...
                if acc_test_1 >= best_acc_test_1:
                    ...
                    print("CURRENT BEST (ACCURACY) FOUND AT EPOCH : {} (EVALUATED WITH {})".format(
                        epoch + 1, "TEST_1"))
                    ...
                    if acc_test_1 == best_acc_test_1:
                        ...
                        if acc_test_2 >= best_acc_test_1_for_2:
                            ...
                            log_best_model_info(
                                path, "epoch : " + str(epoch + 1), best_record_acc_test_1, "accuracy", "test_1")
                            save_best_model(path, self.hierarchical_transformer, self.word_encoder,
                                            self.word_pos_encoder, self.time_delay_encoder, self.optimizer, "accuracy", "test_1")
                ...
                if f_score_test_2 >= best_f_score_test_2:
                    ...
                    print(
                        "CURRENT BEST (F-SCORE) FOUND AT EPOCH : {} (EVALUATED WITH {})".format(epoch + 1, "TEST_2"))
                    ...
                    if f_score_test_2 == best_f_score_test_2:
                        ...
                        if f_score_test_1 >= best_f_score_test_2_for_1:
                            ...
                            log_best_model_info(
                                path, "epoch : " + str(epoch + 1), best_record_f_score_test_2, "f_score", "test_2")
                            save_best_model(path, self.hierarchical_transformer, self.word_encoder,
                                            self.word_pos_encoder, self.time_delay_encoder, self.optimizer, "f_score", "test_2")
                ...
                if acc_test_2 >= best_acc_test_2:
                    ...
                    if acc_test_2 == best_acc_test_2:
                        ...
                        if acc_test_1 >= best_acc_test_2_for_1:
                            ...
                            log_best_model_info(
                                path, "epoch : " + str(epoch + 1), best_record_acc_test_2, "accuracy", "test_2")
                            save_best_model(path, self.hierarchical_transformer, self.word_encoder,
                                            self.word_pos_encoder, self.time_delay_encoder, self.optimizer, "accuracy", "test_2")
                ...
                if f_score_test >= best_f_score_test:
                    print(
                        "CURRENT BEST (F-SCORE) FOUND AT EPOCH : {} (EVALUATED WITH {})".format(epoch + 1, "TEST"))

                    best_f_score_test = f_score_test
                    best_record_f_score_test = record

                    log_best_model_info(
                        path, "epoch : " + str(epoch + 1), best_record_f_score_test, "f_score", "test")
                    save_best_model(path, self.hierarchical_transformer, self.word_encoder,
                                    self.word_pos_encoder, self.time_delay_encoder, self.optimizer, "f_score", "test")
                ...
                if acc_test >= best_acc_test:
                    ...
                    log_best_model_info(
                        path, "epoch : " + str(epoch + 1), best_record_acc_test, "accuracy", "test")
                    save_best_model(path, self.hierarchical_transformer, self.word_encoder,
                                    self.word_pos_encoder, self.time_delay_encoder, self.optimizer, "accuracy", "test")
...
if __name__ == "__main__":
    ...
    for i in range(1):
        ...
        hierarchical_transformer = HierarchicalTransformer.HierarchicalTransformer(
            config)
